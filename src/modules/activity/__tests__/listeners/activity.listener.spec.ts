...
describe('ActivityListener', () => {
  ...
  describe('Board Events', () => {
    it('should log board created event', async () => {
      const event = new BoardCreatedEvent({ id: 'b-1', title: 'Board' } as any, 'u-1');
    ...
    it('should catch error on board created event failure', async () => {
      ...
      await expect(
        listener.handleBoardCreatedEvent(new BoardCreatedEvent({ id: 'b-1' } as any, 'u-1')),
      ).resolves.not.toThrow();
    });

    it('should log board updated event', async () => {
      const event = new BoardUpdatedEvent({ id: 'b-1', title: 'Updated Board' } as any, 'u-1');
    ...
    it('should catch error on board updated event failure', async () => {
      activityRepo.create.mockRejectedValue(new Error('fail'));
      await expect(
        listener.handleBoardUpdatedEvent(new BoardUpdatedEvent({ id: 'b-1' } as any, 'u-1')),
      ).resolves.not.toThrow();
    });
    ...
    it('should catch error on board archived event failure', async () => {
      activityRepo.create.mockRejectedValue(new Error('fail'));
      await expect(
        listener.handleBoardArchivedEvent(new BoardArchivedEvent('b-1', 'ws-1', 'u-1')),
      ).resolves.not.toThrow();
    });

    it('should log board unarchived event', async () => {
      const event = new BoardUnarchivedEvent({ id: 'b-1', title: 'Restored Board' } as any, 'u-1');
    ...
    it('should catch error on board unarchived event failure', async () => {
      activityRepo.create.mockRejectedValue(new Error('fail'));
      await expect(
        listener.handleBoardUnarchivedEvent(new BoardUnarchivedEvent({ id: 'b-1' } as any, 'u-1')),
      ).resolves.not.toThrow();
    });
  });
  ...
  describe('List Events', () => {
    it('should log list created event', async () => {
      const event = new ListCreatedEvent({ id: 'l-1', boardId: 'b-1', title: 'To Do' } as any, 'u-1');
    ...
    it('should catch error on list created event failure', async () => {
      activityRepo.create.mockRejectedValue(new Error('fail'));
      await expect(
        listener.handleListCreatedEvent(new ListCreatedEvent({ id: 'l-1' } as any, 'u-1')),
      ).resolves.not.toThrow();
    });

    it('should log list updated event', async () => {
      const event = new ListUpdatedEvent({ id: 'l-1', boardId: 'b-1', title: 'Doing' } as any, 'u-1');
    ...
    it('should catch error on list updated event failure', async () => {
      activityRepo.create.mockRejectedValue(new Error('fail'));
      await expect(
        listener.handleListUpdatedEvent(new ListUpdatedEvent({ id: 'l-1' } as any, 'u-1')),
      ).resolves.not.toThrow();
    });
    ...
    it('should catch error on list moved event failure', async () => {
      activityRepo.create.mockRejectedValue(new Error('fail'));
      await expect(
        listener.handleListMovedEvent(new ListMovedEvent('l-1', 'b-1', '0|b:', 'u-1')),
      ).resolves.not.toThrow();
    });
    ...
    it('should catch error on list archived event failure', async () => {
      activityRepo.create.mockRejectedValue(new Error('fail'));
      await expect(
        listener.handleListArchivedEvent(new ListArchivedEvent('l-1', 'b-1', 'u-1')),
      ).resolves.not.toThrow();
    });

    it('should log list unarchived event', async () => {
      const event = new ListUnarchivedEvent({ id: 'l-1', boardId: 'b-1', title: 'Restored List' } as any, 'u-1');
    ...
    it('should catch error on list unarchived event failure', async () => {
      activityRepo.create.mockRejectedValue(new Error('fail'));
      await expect(
        listener.handleListUnarchivedEvent(new ListUnarchivedEvent({ id: 'l-1' } as any, 'u-1')),
      ).resolves.not.toThrow();
    });
  });
  ...
  describe('Card Events', () => {
    it('should log card created event', async () => {
      const event = new CardCreatedEvent({ id: 'c-1', title: 'Card 1' } as any, 'b-1', 'l-1', 'u-1');
    ...
    it('should catch error on card created event failure', async () => {
      activityRepo.create.mockRejectedValue(new Error('fail'));
      await expect(
        listener.handleCardCreatedEvent(new CardCreatedEvent({ id: 'c-1' } as any, 'b-1', 'l-1', 'u-1')),
      ).resolves.not.toThrow();
    });
    ...
    it('should catch error on card moved event failure', async () => {
      activityRepo.create.mockRejectedValue(new Error('fail'));
      await expect(
        listener.handleCardMovedEvent(new CardMovedEvent('c-1', 'b-1', 'l-1', 'l-2', '0|b:', 'u-1')),
      ).resolves.not.toThrow();
    });

    it('should log card updated event', async () => {
      const event = new CardUpdatedEvent({ id: 'c-1', title: 'Updated Card' } as any, 'b-1', 'u-1');
    ...
    it('should catch error on card updated event failure', async () => {
      activityRepo.create.mockRejectedValue(new Error('fail'));
      await expect(
        listener.handleCardUpdatedEvent(new CardUpdatedEvent({ id: 'c-1' } as any, 'b-1', 'u-1')),
      ).resolves.not.toThrow();
    });
    ...
    it('should catch error on card archived event failure', async () => {
      activityRepo.create.mockRejectedValue(new Error('fail'));
      await expect(
        listener.handleCardArchivedEvent(new CardArchivedEvent('c-1', 'b-1', 'l-1', 'u-1')),
      ).resolves.not.toThrow();
    });

    it('should log card unarchived event', async () => {
      const event = new CardUnarchivedEvent({ id: 'c-1', title: 'Restored Card' } as any, 'b-1', 'l-1', 'u-1');
    ...
    it('should catch error on card unarchived event failure', async () => {
      activityRepo.create.mockRejectedValue(new Error('fail'));
      await expect(
        listener.handleCardUnarchivedEvent(new CardUnarchivedEvent({ id: 'c-1' } as any, 'b-1', 'l-1', 'u-1')),
      ).resolves.not.toThrow();
    });
  });
  ...
  describe('Comment Events', () => {
    it('should log comment created event', async () => {
      const event = new CommentCreatedEvent({ id: 'comm-1' } as any, 'b-1', 'u-1');
    ...
    it('should catch error on comment created event failure', async () => {
      activityRepo.create.mockRejectedValue(new Error('fail'));
      await expect(
        listener.handleCommentCreatedEvent(new CommentCreatedEvent({ id: 'comm-1' } as any, 'b-1', 'u-1')),
      ).resolves.not.toThrow();
    });
  });
});
